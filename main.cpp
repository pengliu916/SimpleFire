...
SimpleFire								FireSimple = SimpleFire( SUB_TEXTUREWIDTH,SUB_TEXTUREHEIGHT);
									  ...
									  void* pUserContext )
{
	...
	V_RETURN( FireSimple.CreateResource( pd3dDevice ));
	V_RETURN( MultiTexture.CreateResource( pd3dDevice, FireSimple.m_pOutputTextureSRV ));
	return S_OK;
}
...
{
	FireSimple.Update( fElapsedTime );
}
...
{
	FireSimple.Render( pd3dImmediateContext, fTime, fElapsedTime );
...
{
	FireSimple.HandleMessages( hWnd, uMsg, wParam, lParam );
	return 0;
}
